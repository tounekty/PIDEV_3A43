...
public class EmailService {
    ...
    public EmailService() {
        this(
                readSetting(new String[]{"SMTP_HOST", "MAIL_SMTP_HOST"}, "mail.smtp.host", ConfigurationManager.getInstance().getMailSmtpHost()),
                readIntSetting(new String[]{"SMTP_PORT", "MAIL_SMTP_PORT"}, "mail.smtp.port", ConfigurationManager.getInstance().getMailSmtpPort()),
                readSetting(new String[]{"SMTP_USERNAME", "SMTP_USER", "MAIL_SMTP_USERNAME"}, "mail.smtp.username", ConfigurationManager.getInstance().getMailUsername()),
                readSetting(new String[]{"SMTP_PASSWORD", "SMTP_PASSWORD"}, "mail.smtp.password", ConfigurationManager.getInstance().getMailPassword()),
                readSetting(new String[]{"SMTP_FROM", "MAIL_FROM"}, "mail.from.address", ConfigurationManager.getInstance().getMailFromAddress()),
                readBooleanSetting(new String[]{"SMTP_AUTH", "MAIL_SMTP_AUTH"}, "mail.smtp.auth", ConfigurationManager.getInstance().getBooleanProperty("mail.smtp.auth", true)),
                readBooleanSetting(new String[]{"SMTP_STARTTLS", "MAIL_SMTP_STARTTLS"}, "mail.smtp.starttls.enable", ConfigurationManager.getInstance().getBooleanProperty("mail.smtp.starttls.enable", false)),
                readBooleanSetting(new String[]{"SMTP_SSL_ENABLE", "MAIL_SMTP_SSL_ENABLE"}, "mail.smtp.ssl.enable", ConfigurationManager.getInstance().getBooleanProperty("mail.smtp.ssl.enable", true)),
    ...
    public EmailService(String host, int port, String username, String password, String fromAddress,
        ...
        boolean effectiveAuth = auth;
        if (effectiveAuth && (username == null || username.isBlank() || password == null || password.isBlank())) {
            effectiveAuth = false;
        }

        if (isLocalMailhog(host, port)) {
            effectiveAuth = false;
            startTls = false;
            sslEnabled = false;
        }

        this.auth = effectiveAuth;
        this.startTls = startTls;
        this.sslEnabled = sslEnabled;
        this.sslTrust = sslTrust;
    }

    private static boolean isLocalMailhog(String host, int port) {
        if (host == null) {
            return false;
        }
        String normalizedHost = host.trim().toLowerCase();
        return port == 1025 && (normalizedHost.equals("localhost") || normalizedHost.equals("127.0.0.1") || normalizedHost.equals("0.0.0.0") || normalizedHost.contains("mailhog"));
    }
    ...
    private static String readSetting(String[] envKeys, String propertyKey, String defaultValue) {
        ...
        if (value == null || value.isBlank()) {
            for (String envKey : envKeys) {
                ...
                value = AppConfig.get(propertyKey, envKey, "");
                if (value != null && !value.isBlank()) {
                    break;
                }
            }
