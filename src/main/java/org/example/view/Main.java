...
public class Main extends Application {
    ...
    private static final String NAV_CONTAINER = "-fx-background-color: white; -fx-background-radius: 16; -fx-effect: dropshadow(gaussian, rgba(0,0,0,0.06), 15, 0, 0, 5); -fx-padding: 24;";
    private static final String NAV_ACTIVE = "-fx-background-color: #eff6ff; -fx-text-fill: #2563eb; -fx-font-weight: bold; -fx-font-size: 14px; -fx-padding: 14 16; -fx-background-radius: 10; -fx-cursor: hand;";
    private static final String NAV_INACTIVE = "-fx-background-color: transparent; -fx-text-fill: #64748b; -fx-font-size: 14px; -fx-padding: 14 16; -fx-background-radius: 10; -fx-cursor: hand;";
    ...
    private Button frontHomeNavButton;
    ...
    private Button activeAdminNavButton;
    ...
    @FXML private TableColumn<User, Integer> usersIdCol;
    ...
    @FXML private Label homeResourcesCount;
    @FXML private Label homeResourcesIcon;
    ...
    private VBox buildFrontNav() {
        Label title = new Label("NAVIGATION");
        title.setStyle("-fx-font-size: 12px; -fx-font-weight: bold; -fx-text-fill: #94a3b8; -fx-padding: 0 0 8 5; -fx-letter-spacing: 1px;");

        frontHomeNavButton = new Button("🏠 Accueil");
        ...
        for (Button b : List.of(frontHomeNavButton, frontEventsNavButton, frontReservationsNavButton, frontForumNavButton, frontMoodNavButton, frontJournalNavButton, frontResourcesNavButton)) {
            b.setAlignment(Pos.BASELINE_LEFT);
            b.setMaxWidth(Double.MAX_VALUE);
            b.setStyle(NAV_INACTIVE);
        }

        frontHomeNavButton.setOnAction(e -> { updateHomePageStats(); showPage(ensureHomePageLoaded()); setActiveFrontNav(frontHomeNavButton); });
        ...
        box.setStyle(NAV_CONTAINER);
        box.setPrefWidth(260);
        return box;
    }

    private void setFrontNavVisible(boolean visible) {
        if (root == null) {
            return;
        }
        if (!visible) {
        ...
        if (frontNav == null) {
            frontNav = buildFrontNav();
        }
        root.setLeft(frontNav);
        BorderPane.setMargin(frontNav, new Insets(0, 24, 0, 0));
    }

    private void setActiveFrontNav(Button button) {
        if (button == null) {
            return;
        }
        if (activeFrontNavButton != null) {
            activeFrontNavButton.setStyle(NAV_INACTIVE);
        }
        activeFrontNavButton = button;
        activeFrontNavButton.setStyle(NAV_ACTIVE);
    }

    private void syncFrontNavForPage(VBox page) {
        if (page == null) {
            return;
        }
        if (page == homePage) {
            setActiveFrontNav(frontHomeNavButton);
        } else if (page == eventsPage) {
            setActiveFrontNav(frontEventsNavButton);
        } else if (page == reservationsPage) {
    ...
    private void setActiveAdminNav(Button button) {
        ...
        List<Button> adminButtons = Arrays.asList(
                adminDashboardNavButton,
                adminUsersNavButton,
                adminAddEventNavButton,
                adminEventsNavButton,
        ...
        for (Button navButton : adminButtons) {
            if (navButton != null) {
                navButton.setStyle(NAV_INACTIVE);
            }
        }
        activeAdminNavButton = button;
        activeAdminNavButton.setStyle(NAV_ACTIVE);
    }
    ...
    private void populateStatsBox(VBox statsBox) throws SQLException {
        ...
        Label subtitle = new Label("Vue d'ensemble temps reel: activite, engagement et contenu.");
        subtitle.setStyle("-fx-text-fill:#64748b; -fx-font-size:13px; -fx-font-weight:600;");
        statsBox.getChildren().add(title);
        statsBox.getChildren().add(subtitle);

        int eventsTotal = 0;
        int reservationsTotal = 0;
        int forumSubjectsTotal = 0;
        int resourcesTotal = 0;
        int usersTotal = 0;

        try {
            eventsTotal = eventService.getAllEvents().size();
        } catch (SQLException ignored) { }
        try {
            reservationsTotal = reservationService.getTotalReservations();
        } catch (SQLException ignored) { }
        try {
            forumSubjectsTotal = forumService.getAllSubjects().size();
        } catch (SQLException ignored) { }
        try {
            resourcesTotal = resourceService.getAllResources().size();
        } catch (SQLException ignored) { }
        try {
            usersTotal = userController.getAllUsers().size();
        } catch (SQLException ignored) { }

        FlowPane kpiPane = new FlowPane();
        kpiPane.setHgap(16);
        kpiPane.setVgap(16);
        kpiPane.setPrefWrapLength(900);
        kpiPane.setAlignment(Pos.CENTER_LEFT);

        kpiPane.getChildren().add(buildGlobalStatCard("Evenements", String.valueOf(eventsTotal), "#6366f1", "#4f46e5"));
        kpiPane.getChildren().add(buildGlobalStatCard("Reservations", String.valueOf(reservationsTotal), "#0ea5e9", "#0284c7"));
        kpiPane.getChildren().add(buildGlobalStatCard("Sujets Forum", String.valueOf(forumSubjectsTotal), "#ec4899", "#db2777"));
        kpiPane.getChildren().add(buildGlobalStatCard("Ressources", String.valueOf(resourcesTotal), "#22c55e", "#16a34a"));
        kpiPane.getChildren().add(buildGlobalStatCard("Utilisateurs", String.valueOf(usersTotal), "#f59e0b", "#d97706"));
        statsBox.getChildren().add(kpiPane);

        int totalModules = eventsTotal + reservationsTotal + forumSubjectsTotal + resourcesTotal;
        VBox distributionBox = new VBox(10);
        distributionBox.setPadding(new Insets(14));
        distributionBox.setStyle("-fx-background-color: rgba(255,255,255,0.92); -fx-background-radius: 14; -fx-border-color: rgba(120,169,230,0.20); -fx-border-radius: 14;");
        distributionBox.getChildren().add(title("Distribution globale des modules", 16));
        if (totalModules > 0) {
            PieChart modulesPieChart = new PieChart();
            modulesPieChart.setLegendSide(Side.RIGHT);
            modulesPieChart.setPrefSize(560, 260);
            modulesPieChart.setLabelsVisible(true);
            modulesPieChart.getData().add(new PieChart.Data("Evenements (" + eventsTotal + ")", eventsTotal));
            modulesPieChart.getData().add(new PieChart.Data("Reservations (" + reservationsTotal + ")", reservationsTotal));
            modulesPieChart.getData().add(new PieChart.Data("Forum (" + forumSubjectsTotal + ")", forumSubjectsTotal));
            modulesPieChart.getData().add(new PieChart.Data("Ressources (" + resourcesTotal + ")", resourcesTotal));
            distributionBox.getChildren().add(modulesPieChart);
        } else {
            Label emptyGlobal = new Label("Aucune donnee globale disponible pour le moment.");
            emptyGlobal.setStyle("-fx-text-fill:#64748b; -fx-font-size:13px; -fx-font-weight:600;");
            distributionBox.getChildren().add(emptyGlobal);
        }
        statsBox.getChildren().add(distributionBox);
        ...
        if (!reservationsByCategory.isEmpty()) {
            ...
            resPieChart.setTitle("Repartition des reservations");
            resPieChart.setLegendSide(javafx.geometry.Side.RIGHT);
            resPieChart.setPrefSize(420, 300);
            for (Map.Entry<String, Integer> entry : reservationsByCategory.entrySet()) {
                String cat = (entry.getKey() == null || entry.getKey().trim().isEmpty()) ? "Non categorise" : entry.getKey().trim();
                int count = entry.getValue() == null ? 0 : entry.getValue();
                if (count > 0) {
                    resPieChart.getData().add(new javafx.scene.chart.PieChart.Data(cat + " (" + count + ")", count));
                }
            }
            if (resPieChart.getData().isEmpty()) {
                resBox.getChildren().add(new Label("Aucune reservation categoriee pour le moment."));
            } else {
        ...
        if (!eventsByCategory.isEmpty()) {
            ...
            evtPieChart.setTitle("Repartition des evenements");
            evtPieChart.setLegendSide(javafx.geometry.Side.RIGHT);
            evtPieChart.setPrefSize(420, 300);
            for (Map.Entry<String, Integer> entry : eventsByCategory.entrySet()) {
                String cat = (entry.getKey() == null || entry.getKey().trim().isEmpty()) ? "Non categorise" : entry.getKey().trim();
                int count = entry.getValue() == null ? 0 : entry.getValue();
                if (count > 0) {
                    evtPieChart.getData().add(new javafx.scene.chart.PieChart.Data(cat + " (" + count + ")", count));
                }
            }
            if (evtPieChart.getData().isEmpty()) {
                evtBox.getChildren().add(new Label("Aucun evenement categorie pour le moment."));
            } else {
        ...
        chartsRow.getChildren().addAll(resBox, evtBox);
        statsBox.getChildren().add(chartsRow);
    }

    private VBox buildGlobalStatCard(String label, String value, String startColor, String endColor) {
        Label valueLabel = new Label(value);
        valueLabel.setStyle("-fx-text-fill: white; -fx-font-size: 28px; -fx-font-weight: 800;");
        Label textLabel = new Label(label);
        textLabel.setStyle("-fx-text-fill: rgba(255,255,255,0.92); -fx-font-size: 13px; -fx-font-weight: 700;");

        VBox card = new VBox(8, valueLabel, textLabel);
        card.setAlignment(Pos.CENTER_LEFT);
        card.setPrefWidth(170);
        card.setMinHeight(96);
        card.setPadding(new Insets(16));
        card.setStyle("-fx-background-color: linear-gradient(to right, " + startColor + ", " + endColor + ");"
    ...
    private void initHomePage() {
        ...
        if (homeResourcesIcon != null) homeResourcesIcon.setText("📚");
        ...
        if (homeResourcesCount != null) homeResourcesCount.setText("0");
    }
    ...
    private void updateHomePageStats() {
        if (homeEventsCount != null && homeForumCount != null && homeReservationsCount != null) {
            try {
                ...
                if (homeResourcesCount != null) {
                    int resourcesCount = resourceService.getAllResources().size();
                    homeResourcesCount.setText(String.valueOf(resourcesCount));
                }
            } catch (SQLException e) {
                e.printStackTrace();
            }
    ...
    private void handleBanAdminUser(User user) {
        ...
        try {
            ...
            if (banResult.clearBan) {
                showInfo("User unbanned", user.getFullName() + " is now active again.");
            } else {
                showInfo("User banned", user.getFullName() + " has been banned until " + banResult.selectedDate.format(java.time.format.DateTimeFormatter.ofPattern("dd/MM/yyyy")) + ".");
            }
            loadAdminUsers();
        } catch (SQLException e) {
    ...
    private void handleDeleteAdminUser(User user) {
        if (user != null) {
            ...
            alert.setContentText("Confirm deletion of " + user.getFullName() + "?");
    ...
    private BanDialogResult showBanUntilDialog(User user) {
        ...
        dialog.setHeaderText("Choose a ban expiry date or clear the ban for " + user.getFullName() + ".");
                ...
                protected Void call() throws Exception {
                    try {
                        ...
                        if (subject != null && !subject.isEmpty()) {
                            ...
                            // Post-login content can be loaded after the landing page is visible.
                            Platform.runLater(() -> {
                                try { loadEvents(); } catch (Exception ignored) {}
                                try { loadReservations(); } catch (Exception ignored) {}
                                try { loadForumSubjects(); } catch (Exception ignored) {}
                                try { updateHomePageStats(); } catch (Exception ignored) {}
                                if (user.isAdmin()) {
                                    try { loadAdminUsers(); } catch (Exception ignored) {}
                                    try { resetAdminUserForm(); } catch (Exception ignored) {}
                                }
                            });
                        } else {
    ...
    private void attemptLogin() {
        ...
        try {
            ...
            // Show the landing page first, then load the dashboard content.
    ...
    private void applyRole() {
        ...
        // Front office uses a left sidebar navigation; back office uses the admin dashboard navigation.
        setFrontNavVisible(currentUser != null && !backOffice);
    ...
    private void refreshAccountMenuLabel() {
        ...
        accountMenuButton.setText(currentUser.getFullName());
    }
    ...
    private VBox buildAdminDashboardPage() {
        ...
        if (adminDashboardNavButton != null) {
            adminDashboardNavButton.setOnAction(e -> {
                if (adminContentPane != null && adminDashboardStatsPage != null) {
                    ...
                    setActiveAdminNav(adminDashboardNavButton);
                }
            });
        }
        if (adminUsersNavButton != null) {
            adminUsersNavButton.setOnAction(e -> {
                showAdminUsersListPage();
                setActiveAdminNav(adminUsersNavButton);
            });
        }
        if (adminAddEventNavButton != null) {
            adminAddEventNavButton.setOnAction(e -> {
                showAdminContentPage(formPage, this::resetForm);
                setActiveAdminNav(adminAddEventNavButton);
            });
        }
        ...
        if (adminReservationsNavButton != null) {
            adminReservationsNavButton.setOnAction(e -> {
                showAdminContentPage(reservationsPage, this::loadReservations);
                setActiveAdminNav(adminReservationsNavButton);
            });
        }
        if (adminForumNavButton != null) {
            adminForumNavButton.setOnAction(e -> {
                showAdminContentPage(forumPage, this::loadForumSubjects);
                setActiveAdminNav(adminForumNavButton);
            });
        }
        if (adminResourcesNavButton != null) {
            adminResourcesNavButton.setOnAction(e -> {
                showAdminResourcesPage();
                setActiveAdminNav(adminResourcesNavButton);
            });
        }
    ...
    private void initAdminUsersListPage() {
        ...
        if (usersIdCol != null) {
            usersIdCol.setCellValueFactory(data -> new javafx.beans.property.SimpleObjectProperty<>(data.getValue().getId()));
        }
        ...
        if (usersStatusCol != null) {
            usersStatusCol.setCellValueFactory(data -> {
                ...
                if (!u.isEmailVerified()) {
                    ...
                    return new SimpleStringProperty("Banned until " + u.getBannedUntil().format(f));
                }
    ...
    private void applyAdminUsersFilter() {
        ...
        adminUsersFilteredList.setPredicate(user -> {
            ...
            return safeContains(user.getFirstName(), keyword)
    ...
    private void fillAdminUserForm(User user) {
        editingAdminUser = user;
            adminUserFormTitle.setText("Edit User");
    ...
    private void saveAdminUser() {
        ...
        try {
            if (editingAdminUser == null) {
                userController.createUser(email, firstName, lastName, password, role, bannedUntil);
                ...
                userController.updateUser(editingAdminUser.getId(), email, firstName, lastName, password, role, bannedUntil);
    ...
    private void deleteSelectedAdminUser() {
        ...
        alert.setContentText("Confirm deletion of " + selected.getFullName() + "?");
    ...
    private void openEditProfileDialog() {
        ...
        // Custom Title Header
        Label titleLbl = new Label("Profile Settings");
        titleLbl.setStyle("-fx-font-size: 20px; -fx-font-weight: 800; -fx-text-fill: #1e293b;");
        Label subtitleLbl = new Label("Update your account information");
        subtitleLbl.setStyle("-fx-font-size: 13px; -fx-text-fill: #64748b;");
        VBox header = new VBox(2, titleLbl, subtitleLbl);
        header.setPadding(new javafx.geometry.Insets(0, 0, 16, 0));

        // Create standard styled text fields
        String tfStyle = "-fx-background-color: #f8fafc; -fx-border-color: #e2e8f0; -fx-border-radius: 8; -fx-background-radius: 8; -fx-border-width: 1.5; -fx-padding: 10 14; -fx-font-size: 14px; -fx-pref-width: 250;";
        ...
        java.util.function.BiConsumer<Integer, String> addLbl = (row, text) -> {
            Label l = new Label(text);
            l.setStyle("-fx-font-size: 13px; -fx-font-weight: bold; -fx-text-fill: #475569;");
            form.add(l, 0, row);
        };


        addLbl.accept(1, "Email Address"); form.add(emailField, 1, 1);
        addLbl.accept(2, "First Name"); form.add(firstNameField, 1, 2);
        addLbl.accept(3, "Last Name"); form.add(lastNameField, 1, 3);
        addLbl.accept(4, "New Password"); form.add(newPasswordField, 1, 4);

        // Face ID Section
        VBox faceIdSection = new VBox(8);
        faceIdSection.setStyle("-fx-background-color: #f8fafc; -fx-border-color: #e2e8f0; -fx-border-radius: 10; -fx-padding: 16; -fx-margin-top: 10;");
        Label faceIdTitle = new Label("Biometric Security");
        faceIdTitle.setStyle("-fx-font-weight: 800; -fx-text-fill: #1e293b;");
        Label faceIdDesc = new Label("Enable Face ID to log in securely without a password.");
        faceIdDesc.setStyle("-fx-font-size: 12px; -fx-text-fill: #64748b;");
        
        Button faceIdToggleBtn = new Button(currentUser.isFaceIdEnabled() ? "Desactiver Face ID" : "Activer Face ID");
        faceIdToggleBtn.setStyle(currentUser.isFaceIdEnabled() 
            ? "-fx-background-color: #fee2e2; -fx-text-fill: #dc2626; -fx-font-weight: bold; -fx-background-radius: 8; -fx-padding: 8 16; -fx-cursor: hand;"
            : "-fx-background-color: #6366f1; -fx-text-fill: white; -fx-font-weight: bold; -fx-background-radius: 8; -fx-padding: 8 16; -fx-cursor: hand;");
        faceIdToggleBtn.setOnAction(e -> handleToggleFaceId(faceIdToggleBtn));
        
        faceIdSection.getChildren().addAll(faceIdTitle, faceIdDesc, faceIdToggleBtn);

        // Container
        VBox contentContainer = new VBox(20, header, form, faceIdSection);
        contentContainer.setStyle("-fx-background-color: white; -fx-padding: 32; -fx-background-radius: 16; -fx-border-radius: 16; -fx-border-color: #e2e8f0; -fx-border-width: 1; -fx-effect: dropshadow(gaussian, rgba(15,23,42,0.1), 24, 0, 0, 8);");
        ...
        if (saveBtnNode != null) {
            saveBtnNode.setStyle("-fx-background-color: #6366f1; -fx-text-fill: white; -fx-font-weight: bold; -fx-background-radius: 8; -fx-padding: 8 20; -fx-cursor: hand;");
        }
        Node cancelBtnNode = dialog.getDialogPane().lookupButton(cancelBtnType);
        if (cancelBtnNode != null) {
            cancelBtnNode.setStyle("-fx-background-color: white; -fx-text-fill: #475569; -fx-border-color: #cbd5e1; -fx-border-radius: 8; -fx-background-radius: 8; -fx-font-weight: bold; -fx-padding: 8 20; -fx-cursor: hand;");
        }
        ...
        if (email.isBlank()) {
            showWarning("Email est obligatoire.");
            return;
        }
        if (!isValidEmail(email)) {
        ...
        try {
            userController.updateUser(currentUser.getId(), email, firstName, lastName,
    ...
    private void showFaceCaptureDialog(String titleText, Consumer<String> onCapture) {
        ...
        StackPane cameraStack = new StackPane(imageView, statusLabel);
        cameraStack.setAlignment(Pos.CENTER);
        cameraStack.setPrefSize(400, 300);
        cameraStack.setStyle("-fx-background-color: #1e293b; -fx-background-radius: 8;");

        VBox content = new VBox(15, cameraStack);
        content.setAlignment(Pos.CENTER);
        content.setStyle("-fx-padding: 20px; -fx-background-color: #0f172a; -fx-background-radius: 12px;");
        ...
        if (webcam != null) {
            Task<Void> cameraTask = new Task<Void>() {
                ...
                protected Void call() throws Exception {
                    try {
                        webcam.open();
                        Platform.runLater(() -> statusLabel.setText("Caméra active"));
            ...
            Task<Void> task = new Task<Void>() {
                ...
                protected Void call() throws Exception {
                    try {
                        ...
                        compreFaceClient.deleteSubject(currentUser.getEmail());
                ...
                Task<Void> task = new Task<Void>() {
                    ...
                    protected Void call() throws Exception {
                        try {
                            ...
                            compreFaceClient.enrollFace(base64Image, currentUser.getEmail());
    ...
    private String notifySubjectMentions(ForumSubject subject) {
        try {
            ...
            Set<String> mentions = mentionNotificationService.extractMentionedEmails(
    ...
    private String notifyMessageMentions(ForumMessage message) {
        try {
            // Debug: show extracted mentions
            Set<String> mentions = mentionNotificationService.extractMentionedEmails(message.getContenu());
            if (!mentions.isEmpty()) {
    ...
    private void startReplyToMessage(ForumMessage message) {
        ...
        String author = message.isAnonymous() ? "Anonyme" : (message.getAuthorName() == null ? "Utilisateur" : message.getAuthorName());
    ...
    private void edit(Event event) {
        if (currentUser == null || !currentUser.isAdmin()) { showError("Acces refuse", "Seul l'admin peut modifier."); return; }
            editingEvent = event; formTitle.setText("Modifier l'evenement"); saveButton.setText("Enregistrer");
    ...
    private void showPage(VBox page) {
        ...
        if (currentUser != null && !isBackOfficeUser(currentUser)) {
            syncFrontNavForPage(page);
        }
    }
    ...
    private class ReservationCell extends ListCell<ReservationRecord> {
        @Override protected void updateItem(ReservationRecord r, boolean empty) {
            ...
            VBox box = new VBox(8,
                    ...
                    new Label("Etudiant: " + (r.getAuthorName() == null ? "inconnu" : r.getAuthorName())),
    ...
    private class ForumSubjectCell extends ListCell<ForumSubject> {
        @Override protected void updateItem(ForumSubject subject, boolean empty) {
            ...
            String author = subject.isAnonymous() ? "Anonyme" : (subject.getAuthorName() == null ? "Utilisateur #" + subject.getIdUser() : subject.getAuthorName());
