...
public class AppointmentNotificationService {
    ...
    private void sendAcceptanceEmail(Appointment appointment) {
        ...
        ZoomMeetingData zoomMeetingData = resolveZoomMeetingData(appointment);
        String zoomLink = zoomMeetingData == null ? null : zoomMeetingData.getJoinUrl();
    ...
    private String buildAcceptanceBody(Appointment appointment, User recipient, User student, User psychologue, String zoomLink) {
        ...
        appendAppointmentSummary(body, appointment, student, psychologue);
        if (appointment != null && appointment.getZoomCreatedAt() != null) {
            body.append("Zoom créé le: ").append(formatDateTime(appointment.getZoomCreatedAt())).append("\n");
        }
    ...
    private ZoomMeetingData resolveZoomMeetingData(Appointment appointment) {
        if (appointment == null || !isOnline(appointment)) {
            return null;
        }

        if (!isBlank(appointment.getZoomJoinUrl())) {
            return new ZoomMeetingData(appointment.getZoomMeetingId(), appointment.getZoomJoinUrl(), appointment.getZoomCreatedAt());
        }

        Optional<ZoomMeetingData> created = zoomMeetingService.createMeeting(appointment);
        if (created.isPresent()) {
            ZoomMeetingData data = created.get();
            appointment.setZoomMeetingId(data.getMeetingId());
            appointment.setZoomJoinUrl(data.getJoinUrl());
            appointment.setZoomCreatedAt(data.getCreatedAt());
            appointmentService.updateZoomMeetingDetails(
                appointment.getId(),
                data.getMeetingId(),
                data.getJoinUrl(),
                data.getCreatedAt()
