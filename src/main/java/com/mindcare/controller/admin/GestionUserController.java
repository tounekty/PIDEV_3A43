...
public class GestionUserController {
    ...
    private void applyFilters() {
        ...
        ObservableList<User> filteredUsers = users.filtered(user -> {
            boolean matchesQuery = query.isBlank()
                || safeLower(user.getFullName()).contains(query)
                || safeLower(user.getUsername()).contains(query)
    ...
    private void showUserDetails(User user) {
        ...
        grid.setHgap(14);
        grid.setVgap(8);
        grid.addRow(0, detailKey("Username"), detailValue(valueOrDash(user.getUsername())));
